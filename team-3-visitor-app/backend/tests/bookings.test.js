          ...
          .end(function (err, res) {
            ...
            if(Object.keys(data.bookings).length) { // If there are any bookings
                for(var i = 0; i < data.bookings.length; i++) {
                    expect(Object.keys(data.bookings[i]).length).to.be.equal(7); // Booking should contain all 7 elements, same as listed below:
                    ...
                    expect(data.bookings[i]).to.have.property("activityId"); // Booking should contain an activity id
                    expect(data.bookings[i]).to.have.property("ticketAmount"); // Booking should contain the amount of tickets
                    expect(data.bookings[i]).to.have.property("timeslot"); // Booking should contain a timeslot
